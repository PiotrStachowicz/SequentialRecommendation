...
class Data(Dataset):
    def __init__(self, data, all_train, opt, n_node, sw=[2]):
        ...
        # print(len(data[4]))
        # print(len(data[5]))
        # najprawdopodobniej oryginalne dane mają błąd. zgadywana struktura: train. valid, target; test - valid - target??
        self.n_node = n_node
        inputs, mask, max_len, max_edge_num = handle_data(data[0], sw, opt)
        self.inputs = np.asarray(inputs)
        self.targets = np.asarray(data[-1])  # orginalnie był indeks 1
