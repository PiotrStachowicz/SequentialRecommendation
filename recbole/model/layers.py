...
class NOVAMultiHeadAttention(nn.Module):
    ...
    def forward(self, pure_input_tensor, input_tensor, attention_mask):
        mixed_query_layer = self.query(input_tensor)
        mixed_key_layer = self.key(input_tensor)
        mixed_value_layer = self.value(pure_input_tensor)
...
class NOVATransformerLayer(nn.Module):
    ...
    def forward(self, pure_hidden_states, fused_hidden_states, attention_mask):
        attention_output = self.multi_head_attention(pure_hidden_states, fused_hidden_states, attention_mask)
        feedforward_output = self.feed_forward(attention_output)
        return feedforward_output
...
class NOVATransformerEncoder(nn.Module):
    ...
    def forward(self, pure_hidden_states, fused_hidden_states, attention_mask, output_all_encoded_layers=True):
        """
        Args:
            hidden_states (torch.Tensor): the input of the TransformerEncoder
            attention_mask (torch.Tensor): the attention mask for the input hidden_states
        ...
        all_encoder_layers = []
        for layer_module in self.layer:
            pure_hidden_states = layer_module(pure_hidden_states, fused_hidden_states, attention_mask)
            if output_all_encoded_layers:
                all_encoder_layers.append(pure_hidden_states)
        if not output_all_encoded_layers:
            all_encoder_layers.append(pure_hidden_states)
        return all_encoder_layers
