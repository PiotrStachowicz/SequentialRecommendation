...
class MLP(nn.Module):
    def __init__(self, input_dim=4096, output_dim=128, dropout=0.1, device='cuda'):
        ...
        module_list = []
        # TODO: tworzenie module list psuje w przypadku wejścia o pożądanym wymiarze
...
class SASRecD_Denoising(SequentialRecommender):
    """
    ...
    def __init__(self, config, dataset):
        super(SASRecD_Denoising, self).__init__(config, dataset)
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        for i, feature_embed_layer in enumerate(self.feature_embed_layer_list):
            if self.feature_type[i] == 'static':
                static_embedding = feature_embed_layer(item_seq)  # TODO: czy MLP się zepsuje jak `.unsqueeze(-2)` przeniosę ignorancko na później?
