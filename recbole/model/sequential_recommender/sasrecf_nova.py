...
class SASRecF_NOVA(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        super(SASRecF_NOVA, self).__init__(config, dataset)
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        input_emb = self.concat_layer(input_concat)  # TUTAJ NASTĘPUJE FUZJA
        input_emb = input_emb + position_embedding
        input_emb = self.LayerNorm(input_emb)
        input_emb = self.dropout(input_emb)

        extended_attention_mask = self.get_attention_mask(item_seq)
        trm_output = self.trm_encoder(item_emb, input_emb, extended_attention_mask, output_all_encoded_layers=True)
        output = trm_output[-1]
